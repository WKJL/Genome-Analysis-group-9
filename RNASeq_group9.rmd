...
```{r eval=FALSE}
cont.matrix_liver <- makeContrasts("Cirrhosis-Control", levels=design_liver) 
cont.matrix_liver

cont.matrix_kupffer <- makeContrasts("Cirrhosis-Control", levels=design_kupffer) 
cont.matrix_kupffer
```
...
```{r}
qlf.liver <- glmQLFTest(fit.liver, contrast = cont.matrix_liver[, 1]) # Quasi-likelihood F-tests
qlf.kupffer <- glmQLFTest(fit.kupffer, contrast = cont.matrix_kupffer[, 1])
```
...
write.table(Gene.symbol.background, file="Gene_symbol_background1.txt", 
            col.names=F, quote=F, row.names=F)

# Select Gene symbols for the set of interest
Gene.symbol.sign <- res.liver$Gene.symbol[(res.liver$FDR<0.05) & (res.liver$logFC<0)]
...
write.table(Gene.symbol.sign, file="Entrez_id_sign.neg.txt", 
            col.names=F, quote=F, row.names=F)
```
...
```{r}
htmltools::includeHTML('Report_wg_results.html')
```
